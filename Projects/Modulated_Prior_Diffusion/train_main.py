...
#
# Revision history:
# - Restored the original main-branch training path to use pre-resized ISIC data
#   from `--train_data_root` and `--eval_data_root` instead of hard-coded paths.
# - Replaced the per-sample resize transform with tensor conversion plus
#   normalization only, because the dataset is now preprocessed on disk.
# - Renamed the MPD mixing knob to `--mpd_w` to distinguish it from sampler
#   guidance.
# - Split sampler guidance into `--sampler_w`.
# - Reduced MPD execution to a single run per invocation by setting
#   `mpd_w_values = [args.mpd_w]`.
# - Renamed MPD result/checkpoint directories from `w_...` to `mpd_w_...` so
#   the on-disk names match the public MPD argument.
#
# Usage:
#   python train_main.py \
#     --exp isic_original_mpd_repro_128_e500 \
#     --train_data_root /work-pvc/macw1030/isic_mpd_png_128 \
#     --eval_data_root /work-pvc/macw1030/isic_mpd_png_128 \
#     --img_size 128 \
#     --mpd_w 0.9 \
#     --epochs 500 \
#     --eval_interval 50 \
...
#     )
    ...
    train_ds = CustomImageDatasetCondition_MPD(
    ...
    for mpd_w in mpd_w_values:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.eval_interval == 0:
                ...
                valid_ds  =CustomImageDatasetCondition_MPD(
...
#                 }, os.path.join(save_root, f"model_epoch{eval_epoch}_lr{args.lr:.1e}_full.pth"))
                ...
                result_dir = os.path.join('result', args.exp, f"mpd_w_{mpd_w:.1f}_lr{args.lr:.1e}")
                os.makedirs(result_dir, exist_ok=True)
                save_image(output, os.path.join(result_dir, f'epoch{eval_epoch}_lr{args.lr:.1e}_mpd_w{mpd_w:.1f}_image.png'))

                # 儲存模型
                save_root = os.path.join('checkpoint', args.exp, f"mpd_w_{mpd_w:.1f}_lr{args.lr:.1e}")
                os.makedirs(save_root, exist_ok=True)
                torch.save({
                    'epoch': epoch,
                    'model': model.state_dict(),
                    'optimizer': optimizer.state_dict(),
                }, os.path.join(save_root, f"model_epoch{eval_epoch}_lr{args.lr:.1e}_mpd_w{mpd_w:.1f}.pth"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data_root', type=str, default='/work-pvc/macw1030/isic_mpd_png_128', help='training dataset root')
    parser.add_argument('--eval_data_root', type=str, default='/work-pvc/macw1030/isic_mpd_png_128', help='evaluation dataset root')
    ...
    parser.add_argument('--mpd_w', type=float, default=0.9, help='MPD mixing strength for training and validation initialization')
    parser.add_argument('--sampler_w', type=float, default=5, help='sampler guidance strength')
