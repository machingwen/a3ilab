...
class MaskEncoderCNN(nn.Module): # 暫時加的，如果不能用可以刪掉
    def __init__(self, in_channels=1, out_channels=128):  # out_channels 建議設為 model_channels
        super().__init__()
...
#         )
    ...
    def forward(self, x, timesteps=None, c1=None, c2=None, force_drop_ids=False, **kwargs):
        ...
        if self.num_atr is not None and self.num_obj is not None:
            ...
            #print(f"C1 after embedding shape: {c1.shape}")
            #c2 = self.obj_emb(c2, force_drop_ids=force_drop_ids)[:, None, :]
            #print(f"C1 after embedding shape: {c1.shape}")
            #print(f"C2 after embedding shape: {c2.shape}")
            if self.concat:
...
class UNetAttentionImage(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
...
#         )
    ...
    def forward(self, x, timesteps=None, c2=None, force_drop_ids=False, **kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
