...
class ImageConditionalGaussianDiffusionTrainer_w(nn.Module):
...
class ImageConditionalGaussianDiffusionTrainer(nn.Module):
...
class GPCDConcatGaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0, c2):
        ...
        epsilon_theta = self.model(x_t, t, c2)
        loss = F.mse_loss(epsilon_theta, epsilon, reduction="none")
        return loss
...
class DDIMSampler(nn.Module):
    ...
    @torch.no_grad()
    def sample_one_step(self, x_t, time_step: int, atr: torch.LongTensor, prev_time_step: int, eta: float):
        t = torch.full((x_t.shape[0],), time_step, device=x_t.device, dtype=torch.long)
    #def sample_one_step(self, x_t, time_step: int, atr: torch.LongTensor, obj: torch.LongTensor, prev_time_step: int, eta: float):
        ...
        epsilon_theta_t = self.model(x_t, t, atr )
        #unc_epsilon_theta_t = self.model(x_t, t, atr, obj, force_drop_ids=True)
        unc_epsilon_theta_t = self.model(x_t, t, atr, force_drop_ids=True)
    ...
    @torch.no_grad()
    def forward(self, x_t, atr: torch.LongTensor,  steps: int = 1, method="linear", eta=0.0,
    ...
    #             only_return_x_0: bool = True, interval: int = 1):
        ...
        with tqdm(reversed(range(0, steps)), colour="#6565b5", total=steps) as sampling_steps:
            for i in sampling_steps:
                ...
                x_t = self.sample_one_step(x_t, time_steps[i], atr, time_steps_prev[i], eta)
...
class DDIMSamplerImage(nn.Module):
    def __init__(self, model, beta: Tuple[int, int], T: int, w: float, schedule="linear"):
        super().__init__()
        self.model = model
        self.T = T
...
class DDIMSamplerImageGPCDConcat(nn.Module):
    def __init__(self, model, beta: Tuple[int, int], T: int, w: float, schedule="linear"):
        super().__init__()
        self.model = model
        self.T = T
    ...
    @torch.no_grad()
    def sample_one_step(self, x_t, c2, time_step: int, prev_time_step: int, eta: float):
        t = torch.full((x_t.shape[0],), time_step, device=x_t.device, dtype=torch.long)
        prev_t = torch.full((x_t.shape[0],), prev_time_step, device=x_t.device, dtype=torch.long)
        ...
        # predict noise with concat-downsized reference conditioning
        epsilon_theta_t = self.model(x_t, t, c2)
    ...
    @torch.no_grad()
    def forward(self, x_t, c2, steps: int = 1, method="linear", eta=0.0,
                    only_return_x_0: bool = True, interval: int = 1):
    
        """
        Parameters:
            x_t: Standard Gaussian noise. A tensor with shape (batch_size, channels, height, width).
            c2: Reference image. A tensor with shape (batch_size, channels, height, width).
            steps: Sampling steps.
            method: Sampling method, can be "linear" or "quadratic".
            eta: Coefficients of sigma parameters in the paper. The value 0 indicates DDIM, 1 indicates DDPM.
            only_return_x_0: Determines whether the image is saved during the sampling process. if True,
        ...
        with tqdm(reversed(range(0, steps)), colour="#6565b5", total=steps) as sampling_steps:
            for i in sampling_steps:
                x_t = self.sample_one_step(x_t, c2, time_steps[i], time_steps_prev[i], eta)
                if not only_return_x_0 and ((steps - i) % interval == 0 or i == 0):
                    x.append(torch.clip(x_t, -1.0, 1.0))
