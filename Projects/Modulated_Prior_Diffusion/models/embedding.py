...
class ConditionalEmbedding(nn.Module):
    ...
    def __init__(self, channels: int, d_model: int, emb_dim: int, dropout_prob: float = 0.1):
        """
        ConditionalEmbedding 用於將圖片數據轉換為嵌入向量
        :param channels: 圖片的通道數，例如 RGB 圖片為 3
        :param d_model: 中間特徵向量的維度
        :param emb_dim: 最終嵌入向量的維度
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=d_model, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(d_model),
            Swish(),
            nn.AdaptiveAvgPool2d((1, 1))  # 將高寬降為 1x1
        )
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        """
        將圖片數據轉換為嵌入向量
        :param images: 圖片數據，形狀為 [batch_size, channels, height, width]
        :return: 嵌入向量，形狀為 [batch_size, emb_dim]
        """
        # 提取卷積特徵
        features = self.conv(images)  # [batch_size, d_model, 1, 1]
        features = features.view(features.size(0), -1)  # [batch_size, d_model]
        
        # 嵌入壓縮
        embeddings = self.emb(features)  # [batch_size, emb_dim]
