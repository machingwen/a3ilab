...
def initial_values(train_com_loader, feature_extractor, n_inducing_points):
    steps = 10
    f_X_samples = []
    com_dataloader_iterator = iter(train_com_loader)
    
    with torch.no_grad():
        for i in range(steps):
            try:
                data1 = next(com_dataloader_iterator)
            except StopIteration:
                com_dataloader_iterator = iter(train_com_loader)
                data1 = next(com_dataloader_iterator)
            X_sample, label, _, _ = data1
            
            if torch.cuda.is_available():
                X_sample = X_sample.cuda()
                feature_extractor = feature_extractor.cuda()

            f_X_samples.append(feature_extractor(X_sample)[1].cpu())
...
def initial_values2(train_com_loader, feature_extractor, n_inducing_points):
    steps = 10
    f_X_samples = []
    com_dataloader_iterator = iter(train_com_loader)
...
def initial_values3(train_com_loader, feature_extractor, n_inducing_points):
    steps = 10
    f_X_samples = []
    com_dataloader_iterator = iter(train_com_loader)
...
#         f_X_samples.numpy(), n_inducing_points
#     )
    initial_inducing_points = torch.rand(n_inducing_points, 960)
    initial_lengthscale = _get_initial_lengthscale(f_X_samples)

    return initial_inducing_points, initial_lengthscale

# def initial_values(train_cls_loader, feature_extractor, n_inducing_points):
#     steps = 10
...
class DKL(gpytorch.Module):
    ...
    def forward(self, x):
        ...
        class_out, features = self.feature_extractor(x)
        
        return class_out, self.com_out(features)
