...
#     test_com_dataset = CustomDataset(val_com_df, transform=test_transform)
    ...
    return input_size ,num_classes ,train_com_loader, train_cls_loader, test_dataset ,train_cls_dataset, train_com_dataset, test_com_dataset, train_com_df, train_df, val_df, test_df

def get_fruit_stage2(root, seed,component_name):
    random.seed(seed)
    input_size = 224
#     num_classes = 23
    normalize = transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
    ...
    train_cls_df = pd.read_csv(f'/root/notebooks/dataset/fruit_dataset_csv/fruit_dataset_EXP1_csv.csv')
    train_com_df = pd.read_csv(f'/root/notebooks/dataset/fruit_dataset_csv/fruit_dataset_EXP2_csv.csv')
    test_df = pd.read_csv(f'/root/notebooks/dataset/fruit_dataset_csv/fruit_dataset_test_csv.csv')
...
#     valDatasetMask = (val_df['component_name'].isin(badComponent)) & (val_df['class'] == 1)
    ...
    train_df, val_df = split_stratified_into_train_val(train_cls_df, stratify_colname='component_name', frac_train=0.7, frac_val=0.3, random_state=seed)
    train_com_df, val_com_df = split_stratified_into_train_val(train_com_df, stratify_colname='component_name', frac_train=0.7, frac_val=0.3, random_state=seed)
...
#     train_com_df = train_com_df.groupby('component_name', group_keys=False).apply(lambda x: x.sample(frac=0.5, random_state=123))
    ...
    test_dataset = CustomDataset(val_df, transform=test_transform)
    test_com_dataset = CustomDataset(val_com_df, transform=test_transform)
#     test_unc_dataset = CustomDataset(test_df, transform=test_transform)
...
#     test_unc_loader = torch.utils.data.DataLoader(
#         test_unc_dataset, batch_size=128, shuffle=False, 
#         num_workers=8, pin_memory=True
#     )
    

    return input_size ,num_classes ,train_com_loader, train_cls_loader, test_loader ,train_cls_dataset, train_com_dataset, test_com_dataset, train_com_df, train_cls_df, val_df, test_df
...
#     test_com_dataset = CustomDataset(val_com_df, transform=test_transform)
    ...
    return input_size ,num_classes ,train_com_loader, train_cls_loader, test_dataset ,train_cls_dataset, train_com_dataset, test_com_dataset, train_com_df, train_cls_df, val_df, test_df
...
#     )
    ...
    print('no bad sample component: ',badComponent)
...
#     test_com_dataset = CustomDataset(val_com_df, transform=test_transform)
    ...
    test_unc_loader = torch.utils.data.DataLoader(
        test_unc_dataset, batch_size=32, shuffle=False, 
        num_workers=8, pin_memory=True,drop_last=True
    )


    return input_size ,num_classes ,train_com_loader, train_cls_loader, test_unc_loader ,train_cls_dataset, train_com_dataset, test_com_dataset, train_com_df, train_cls_df, val_df, test_df
...
#     test_com_dataset = CustomDataset(val_com_df, transform=test_transform)
    ...
    return input_size ,num_classes ,train_com_loader, train_cls_loader, test_dataset ,train_cls_dataset, train_com_dataset, test_com_dataset, train_cls_df, train_com_df, val_df, test_df

def get_mvtecad_tex_s2(root, seed):
...
#     return df
def CreateDataset_for_each_component_gp(seed, train_val_df, component_name, split='train', testing=None):
...
#     test_df_bad = test_df.loc[test_df["class"]!=0].sample(n=50, random_state=123)
    ...
    test_df_bad = test_df.loc[test_df["class"]!=0].groupby('component_name', group_keys=False).apply(lambda x: x.sample(n=20, random_state=123))
#     import pdb;pdb.set_trace()
#     test_df_good = test_df.loc[test_df["class"]==0].groupby('component_name', group_keys=False).apply(lambda x: x.sample(frac=0.005, random_state=123))
