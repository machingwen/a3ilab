...
class SparsityTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
    	...
    	def hook_fn_train(module, input,output): # train過程中，隨機dropout冷神經元，須配合  hook_fn_count		
    		...
    		#new_input = input[0].clone()  # gemma-2b 的 fc2(down)	
    		tt=tt%18+1
    		#print('hook_fn_train total=',tt)		
    		cp1_tensor = 1-torch.from_numpy(cp1[tt]).to('cuda')
    		cp1_tensor = cp1_tensor.to(input[0].dtype)
    		...
    		nonlocal tmp
    		#g=new_input.sum()
    		gg = torch.abs(input[0]*cp1_tensor).sum()*3
    		#gg=torch.abs(g)*2
    		tmp+=gg	
    		#print('x: ',input[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l1_lr", type=float,default=1e-7, help="l1 Learning rate ")
