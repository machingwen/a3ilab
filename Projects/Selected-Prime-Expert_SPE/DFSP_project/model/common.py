...
#----------------改編內容-----------------
#輸入為text_feature[1962,768] idx[1962,2]
#輸出為 ft>fs+fo
    def decompose_ftfsfo(self, text_feature, idx):
#         import ipdb 
#         ipdb.set_trace()
        l, c = text_feature.shape
        att_idx, obj_idx = idx[:, 0].cpu().numpy(), idx[:, 1].cpu().numpy()
        text_att = torch.zeros( self.attributes, c).cuda()
        text_obj = torch.zeros( self.classes, c).cuda()
