...
def choose_best_three_expert(probs_expert1,probs_expert2,probs_expert3,pairs, targets,targets_pairs, n_bins=10):
    uce_expert1, bin_uncertainties_expert1, bin_errors_expert1, prop_in_bin_values_expert1,bin_n_samples_ep1, bin_variances_ep1 = compute_uce(probs_expert1, targets, n_bins)
    uce_expert2, bin_uncertainties_expert2, bin_errors_expert2, prop_in_bin_values_expert2,bin_n_samples_ep2, bin_variances_ep2 = compute_uce(probs_expert2, targets_pairs, n_bins)
...
def cal_all_stats(S_attr_exp1,all_logits,all_attr_gt,all_pair_gt,pairs,yfs,stats):
    ...
    #計算table準確度
    table_pred_ep12 = choose_best_expert(S_attr_exp1, S_pair_exp2, all_attr_gt,all_pair_gt,pairs)
    table_acc_ep12 = accuracy(table_pred_ep12,all_attr_gt)

    table_expert_ep13 = choose_best_expert_ex(S_attr_exp1, S_attr_exp3, all_attr_gt)
    table_acc_ep13 = accuracy(table_expert_ep13,all_attr_gt)
    
    table_pred_ep23 = choose_best_expert(S_attr_exp3, S_pair_exp2, all_attr_gt,all_pair_gt,pairs)
    table_acc_ep23 = accuracy(table_pred_ep23,all_attr_gt)
    
    # 計算3位專家綜合準確度
    tabel_pred_ep123 = choose_best_three_expert(S_attr_exp1,S_pair_exp2,S_attr_exp3,pairs,all_attr_gt,all_pair_gt)
...
#         plot_dot_UCE_diagram(uce_value, bin_uncertainties, bin_errors, 1, test_dataset, config, prop_in_bin_values, bin_n_samples, bin_variances)
    ...
    obj_idxs = [torch.where(pairs[:, 1] == obj)[0] for obj in all_obj_gt]
    # 並將對應的位置將S_all_logits>S_logit_attr_ours>ep3
...
#             plot_dot_UCE_diagram(uce_value, bin_uncertainties, bin_errors, 2, test_dataset, config, prop_in_bin_values, bin_n_samples, bin_variances)
    ...
    if config.open_world == True:
        yfs = ""
        cal_all_stats(S_attr_exp1,all_logits,all_attr_gt,all_pair_gt,pairs,yfs,stats)
    #--------------------------------------
...
def test_yfs_ep2_nfs_ep1(
    ...
    S_pair_exp2 = F.softmax(all_logits_, dim=1)       #這個是yfs_ep2
    S_attr_exp3 = F.softmax(attr_exp3, dim=1)         #這個是yfs_ep3
    
    table_pred_yfs_ep2_nfs_ep1 = choose_best_expert(S_attr_exp1, S_pair_exp2, all_attr_gt,all_pair_gt,pairs)
    table_acc_yfs_ep2_nfs_ep1= accuracy(table_pred_yfs_ep2_nfs_ep1,all_attr_gt)
    
    table_pred_yfs_ep23_nfs_ep1 = choose_best_three_expert(S_attr_exp1,S_pair_exp2,S_attr_exp3,pairs,all_attr_gt,all_pair_gt)
    ...
    if config.open_world and config.threshold is None:
        ...
        with torch.no_grad():
            ...
            results =test__yfs(test_dataset,
                    ...
                    all_pair_gt,
                    config)
            results =test_yfs_ep2_nfs_ep1(test_dataset,
                    results,
                    evaluator,
                    all_logits,        
                    all_logits_,
