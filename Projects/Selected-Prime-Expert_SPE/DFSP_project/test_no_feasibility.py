...
#     plt.savefig(config.save_path + '/plt/'+str(len(test_dataset.pairs))+'/'+test_dataset.phase+'_'+"UCE_model_{}.png".format(model_index + 1))
    ...
    plt.savefig(config.save_path + '/plt/'+str(len(test_dataset.pairs))+'/'+test_dataset.phase+'_'+"UCE_model_{}.png".format(model_index + 1))
### 5/1新增
def choose_best_expert(probs_expert1, probs_expert2, targets, n_bins=10):
...
def accuracy(y_true, y_pred):
    ...
    assert y_true.shape == y_pred.shape, "y_true 和 y_pred 的維度必須相同"
...
def test(
    ...
    #-----------------------------------------
# 初始化 all_logits_attr 张量（此处的形状取决于你的实际需求，假设 all_logits 的形状为 (b, n)）
    b, nattr = all_logits.size(0), len(set(pairs[:, 0].tolist()))
    all_logits_attr = torch.zeros((b, nattr), device=all_logits.device)
    for i, obj_gt in enumerate(all_obj_gt):
        # 找到 pairs 中 obj_idx 等于 obj_gt 的行的索引
        matched_indices = (pairs[:, 1] == obj_gt).nonzero(as_tuple=True)[0]
        # 遍历找到的索引
        for idx in matched_indices:
            # 获取对应的属性索引
            attr_idx = pairs[idx, 0]
            # 从 all_logits 张量中选取相应的列
            selected_logits = all_logits[i, idx]
            # 将选取的列复制到 all_logits_attr 张量中的相应位置
            all_logits_attr[i, attr_idx] = selected_logits
...
#     uce_ep2=uce_and_bin_values[0][0]  
    ...
    table_pred = choose_best_expert(S_logits_expert1, S_logits_expert2, all_attr_gt)
    ...
    attr_acc_vote,attr_acc_ep1_cor,attr_acc_ep2_cor = cal_final_expert(all_logits_attr,logits_attrs_list[1],all_attr_gt)
...
#     stats['attr_acc_ece_ep1'] = ece_ep1
#     stats['attr_acc_ece_ep2'] = ece_ep2
...
###----------------新增預測att--------------
    ...
    stats['attr_acc_ep2_cor'] = attr_acc_ep2_cor
