...
def plot_scatter_all(args,args_due, X, y, X_test, y_test):    
    ...
    colors_per_class[len(set(y))+2] = [0, 255, 255] #黃
    colors_per_class[len(set(y))+3] = [255, 0, 0] # 藍
    colors_per_class[len(set(y))+4] = [238,130,238] #紫
    colors_per_class[len(set(y))+5] = [28,28,28] # 黑
...
if __name__ == "__main__":
    ...
    # training_loader, validation_loader, test_loader = CreateTSNEdataset_regroup_1212(args["random_seed"], tsne=True)
    # training_loader, validation_loader, test_loader = CreateTSNEdatasetRelabel(args["random_seed"], tsne=True)
    training_loader, validation_loader, test_loader = CreateTSNEdatasetRelabel_sixcls(args["random_seed"], tsne=True)
    ...
    for lb in labels_test:
        ...
        if lb == 2:
            component_labels_test_mapping.append(len(set(name_list_train))+2)
        if lb == 3:
            component_labels_test_mapping.append(len(set(name_list_train))+3)
        if lb == 4:
            component_labels_test_mapping.append(len(set(name_list_train))+4)
        if lb == 5:
            component_labels_test_mapping.append(len(set(name_list_train))+5)
    ...
    for lb in labels_test:
        ...
        if lb == 2:
            class_labels_test_mapping.append(len(set(labels_train))+2)
        if lb == 3:
            class_labels_test_mapping.append(len(set(labels_train))+3)
        if lb == 4:
            class_labels_test_mapping.append(len(set(labels_train))+4)
        if lb == 5:
            class_labels_test_mapping.append(len(set(labels_train))+5)
