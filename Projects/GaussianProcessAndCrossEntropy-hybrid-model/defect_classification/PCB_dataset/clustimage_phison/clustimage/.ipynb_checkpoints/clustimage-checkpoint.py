...
#         set_logger(verbose=verbose)
    ...
    def fit_transform(self, X,filename, cluster='agglomerative', evaluate='silhouette', metric='euclidean', linkage='ward', min_clust=3, max_clust=25, cluster_space='high', black_list=None):
        ...
        _ = self.import_data(X, filename, black_list=black_list)
    ...
    def find(self, Xnew, filename, metric=None, k=None, alpha=0.05):
        ...
        # Check whether in is dir, list of files or array-like
        Xnew = self.import_data(Xnew,filename)
        # Compute distance
    ...
    def import_data(self, Xraw,filename, flatten=True, black_list=None):
...
#         if isinstance(Xraw, np.ndarray):
        ...
        self.results['filenames'] = filename
            # Make 2D
#             if len(Xraw.shape)==1:
...
#             Xmapped = self.pca.transform(X, row_labels=filename)
