...
#             import pdb;pdb.set_trace()
            ...
            for idx, (gt, pred, name) in enumerate(list(zip(labels.data, prediction.data, component_name))):
                ...
                if name in train_component_label :
                    old_pred_label.append(pred.item())
                    old_true_label.append(gt.item())
                else :
                    new_pred_label.append(pred.item())
                    new_true_label.append(gt.item())
        ...
        cm = confusion_matrix(new_true_label, new_pred_label,labels=[0, 1, 2, 3, 4, 5])
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/new_com_confusion_matrix_method3_expdet.pdf', dpi=300, bbox_inches='tight')
            # plt.show()

        cm = confusion_matrix(old_true_label, old_pred_label,labels=[0, 1, 2, 3, 4, 5])
        cm_df = pd.DataFrame(cm, index=[f'actually:{label}' for label in defect_types],
                        columns=[f'predicted:{label}' for label in defect_types])
            # 可视化混淆矩阵
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/old_com_confusion_matrix_method3_expdet.pdf', dpi=300, bbox_inches='tight')
            # plt.show()
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/all_com_confusion_matrix_method3_expdet.pdf', dpi=300, bbox_inches='tight')                
...
def EXP_classifier(args_due ,model ,test_loader ,df ,train_component_label, val_component_label, test_component_label ):
    ...
    with torch.no_grad():
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/new_com_confusion_matrix_method3_expcls.pdf', dpi=300, bbox_inches='tight')
            # plt.show()
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/old_com_confusion_matrix_method3_expcls.pdf', dpi=300, bbox_inches='tight')
            # plt.show()
        ...
        # 保存图片
        plt.savefig(f'./output/{args_due.output_inference_dir}/all_com_confusion_matrix_method3_expcls.pdf', dpi=300, bbox_inches='tight')    
...
def HybridExpert(y_pred_expert_detector, y_pred_expert_classifier, y_pred_expert_component, com_pred, y_true, name_label_list,  df, args_due, train_component_label, val_component_label, test_component_label):
    ...
    for idx, (pred_expert_detector, pred_expert_classifier, pred_expert_component, pred_com, gt_label, name) in enumerate(list(zip(y_pred_expert_detector, y_pred_expert_classifier, y_pred_expert_component, com_pred, y_true, name_label_list))):
        ...
        if ((pred_expert_detector==0) and (pred_expert_component!=0)) or ((pred_expert_detector != 0) and (pred_expert_component==0)) or (pred_expert_component ==2) or (name != pred_com):
...
#             elif name in train_component_label :
...
#         else if (pred_expert_detector == pred_expert_component) or ((pred_expert_detector != 0) and (pred_expert_component ==1)):
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        default="PHISON",
        choices=["CIFAR10","CIFAR100","PHISON","PHISON_fourcls",'PHISON_shift','PHISON_broke','PHISON_short','PHISON_regroup','PHISON_regroup2','PHISON_regroup3'],
        help="Pick a dataset",
    )
...
#     input_size, num_classes, train_dataset, test_dataset, train_loader, train_com_loader = ds
    ...
    if args_due.test_inference == True:
        ...
        print("3. Testing with DUE (Expert component)")  # generative model
