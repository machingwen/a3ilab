...
def initial_values(train_cls_loader, feature_extractor, n_inducing_points):
    steps = 30
    f_X_samples = []
    class_dataloader_iterator = iter(train_cls_loader)
    
    with torch.no_grad():
        for i in range(steps):
            try:
                data1 = next(class_dataloader_iterator)
...
class DKL(gpytorch.Module):
    def __init__(self, feature_extractor, gp):#, gp2
        """
...
#         self.com_out = gp2
    ...
    def forward(self, x):
        ...
        return self.class_out(features)#, self.com_out(features)
