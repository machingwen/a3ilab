...
def set_model_threecls(args, args_due,train_com_loader,num_com):
    
    from networks.mobilenetv3_six import SupConMobileNetV3Large
    input_size = 224
    num_classes = num_com
...
def EXP_detector_good(args_due ,model ,test_loader ,df ,train_component_label, val_component_label, test_component_label ):
    ...
    with torch.no_grad():
        for idx, (images, _, labels, _, _, _, _, component_name) in enumerate(tqdm(test_loader)):
...
def EXP_classifier_defect(args_due ,model ,test_loader ,df ,train_component_label, val_component_label, test_component_label ):
    ...
    with torch.no_grad():
        for idx, (images, labels,_, _, _, _, _, component_name) in enumerate(tqdm(test_loader)):
            images = images.float().cuda()
            labels = labels.cuda()
            bsz = labels.shape[0]
...
def EXP_component_good(args_due , model_s1,model ,test_loader ,df ,train_component_label, val_component_label, test_component_label ,likelihood ,TH , TH_2):
...
def HybridExpert2(y_pred_expert_good, y_pred_expert_classifier, y_true_classifier, name_label_list,  df, args_due, train_component_label, val_component_label, test_component_label):
    ...
    for idx, (pred_expert_good, pred_expert_classifier, gt_label, name) in enumerate(list(zip(y_pred_expert_good, y_pred_expert_classifier, y_true_classifier, name_label_list))):

            
        if pred_expert_good == 1 :
        ...
        elif pred_expert_good == 0 :
            # and ((pred_expert_broke == 0) and (pred_expert_short ==0)):

            y_pred_all.append(pred_expert_good)
            y_true_all.append(gt_label)
            gt_label_after_unknown.append(gt_label)
            pred_after_unknown.append(pred_expert_good)
            
            if name in test_component_label :
                new_pred_label.append(pred_expert_good)
                new_true_label.append(gt_label)
            else :
                old_pred_label.append(pred_expert_good)
                old_true_label.append(gt_label)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c_good", "--checkpoint_path_good",
        type=str,
        default ="",      # checkpoint.pth.tar
        help="Path to model's checkpoint."
    )
    parser.add_argument(
        "-c_threecls", "--checkpoint_path_threecls",
        type=str,
        default ="",      # checkpoint.pth.tar
        help="Path to model's checkpoint."
    )
    ...
    parser.add_argument(
        "-gp_c_good", "--gp_checkpoint_path_good",
        type=str,
        default ="",      # checkpoint.pth.tar
        help="Path to model's checkpoint."
    )
...
#     logging.info(f"Loaded checkpoint at {args['checkpoint_path']}")
    ...
    args["checkpoint_path"] = args["checkpoint_path_good"]
    args["gp_checkpoint_path"] = args["gp_checkpoint_path_good"]
...
#     input_size, num_classes, train_dataset, test_dataset, train_loader, train_com_loader = ds
    ...
    # set TH 
    df = pd.read_csv(f'./output/CEdue_s2_good_rd_{args_due.random_seed}/uncertainty.csv')
    TH = df['TH']
    TH_2 = df['TH_2']
    ...
    if args_due.test_inference == True:
        ...
        print("1. Testing with Discriminative Model (Expert good detector)")  # Discriminative model
        y_true, y_pred_expert_detector = EXP_detector_good(args_due ,model ,test_loader, test_component_name_df, train_component_label, val_component_label, test_component_label)
        print("3. Testing with DUE (Expert  good component)")  # Discriminative model
        name_label_list, y_pred_expert_component, com_pred  = EXP_component_good(args_due , model,gp_model ,test_loader, test_component_name_df_Expert2, train_component_label, val_component_label, test_component_label ,likelihood ,TH,TH_2)
        print("3. Testing with Hybrid Expert good") 
        y_true_good,y_pred_good = HybridExpert1(y_pred_expert_detector, y_pred_expert_detector, y_pred_expert_component, com_pred, y_true, name_label_list, test_component_name_df_HybridExpert, args_due, train_component_label, val_component_label, test_component_label)

    
    # defect classifier
    
    args["checkpoint_path"] = args["checkpoint_path_threecls"]
    # Intialize model

    print(num_classes)
    model  = set_model_threecls(args, args_due, train_com_loader , num_classes)

    if args_due.test_inference == True:

        print("2. Testing with Discriminative Model (Expert defect classifier)")  # Discriminative model
        y_true_classifier, y_pred_classifier = EXP_classifier_defect(args_due ,model ,test_loader, test_component_name_df, train_component_label, val_component_label, test_component_label)
        
        
        print("3. Testing with Hybrid Expert") 
        HybridExpert2(y_pred_good, y_pred_classifier, y_true_classifier, name_label_list, test_component_name_df_HybridExpert, args_due, train_component_label, val_component_label, test_component_label)
    else:
