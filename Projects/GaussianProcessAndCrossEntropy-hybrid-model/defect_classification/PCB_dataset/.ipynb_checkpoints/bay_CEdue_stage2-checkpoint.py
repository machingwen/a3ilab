...
pbounds = {
    ...
    'com_15_tau1': (0, 1), 'com_15_tau2': (0, 1),
    'com_16_tau1': (0, 1), 'com_16_tau2': (0, 1),
    'com_17_tau1': (0, 1), 'com_17_tau2': (0, 1),
           }
...
def set_loader(args_due):
    ...
    if args_due.dataset2 == 'phison':
        ...
        if args_due.dataset == 'PHISON':
            train_df, val_df, test_df, train_component_label, val_component_label, test_component_label, train_com_df = CreateDataset_relabel_sixcls(args_due.random_seed, testing=None) 
        ...
        if args_due.dataset == 'PHISON_good':
            train_df, val_df, test_df, train_component_label, val_component_label, test_component_label, train_com_df, _ = CreateDataset_relabel_good(args_due.random_seed,add_test=True, testing=None)
        if args_due.dataset == 'PHISON_shift':
            train_df, val_df, test_df, train_component_label, val_component_label, test_component_label, train_com_df, _ = CreateDataset_relabel_shift(args_due.random_seed,add_test=True, testing=None) 
        if args_due.dataset == 'PHISON_broke':
            train_df, val_df, test_df, train_component_label, val_component_label, test_component_label, train_com_df, _ = CreateDataset_relabel_broke(args_due.random_seed,add_test=True, testing=None) 
...
#     calculatePerformance(df_all, file_name=f'./output/{args_due.output_inference_dir}/DUE_dataset_{args_due.output_inference_dir}_EXP1_ALL_test_set_overkill_and_leakage.csv')
    score = calculatePerformance(df_all, file_name=f'./output/train_val/dataset_{args_due.random_seed}_EXP1+_ALL_train_val.csv')
...
#             import pdb;pdb.set_trace()
    ...
    score = calculatePerformance(df_all, file_name=f'./output/train_val/dataset_{args_due.random_seed}_EXP2+_ALL_train_val.csv')
...
def HybridExpert(y_pred_expert1, y_pred_expert2, y_true, name_label_list,  df, args_due, train_component_label, val_component_label, test_component_label):
    ...
    for idx, (pred_expert1, pred_expert2, gt_label, name) in enumerate(list(zip(y_pred_expert1, y_pred_expert2, y_true, name_label_list))):
        ...
        # if ((pred_expert2 != 0) and (pred_expert1 == 0)) or ((pred_expert2 == 0) and (pred_expert1 != 0)) or (pred_expert2 ==2):
        if (pred_expert2 != pred_expert1 ) or (pred_expert2 ==2):
#             import pdb;pdb.set_trace()
            y_pred_all.append(2)
        ...
        # if ((pred_expert1 == pred_expert2) or ((pred_expert2 != 0) and (pred_expert1 != 0))) and (pred_expert2 !=2):
        elif (pred_expert1 == pred_expert2) and (pred_expert2 !=2):
            ...
            elif pred_expert2 == 0 and gt_label != pred_expert2:
                if isinstance(name, np.int64):
...
def search(com_0_tau1, com_0_tau2, com_1_tau1, com_1_tau2, com_2_tau1, com_2_tau2, com_3_tau1, com_3_tau2, com_4_tau1, com_4_tau2, com_5_tau1, com_5_tau2, com_6_tau1, com_6_tau2, com_7_tau1, com_7_tau2, com_8_tau1, com_8_tau2, com_9_tau1, com_9_tau2, com_10_tau1, com_10_tau2, com_11_tau1, com_11_tau2, com_12_tau1, com_12_tau2, com_13_tau1, com_13_tau2, com_14_tau1, com_14_tau2, com_15_tau1, com_15_tau2, com_16_tau1, com_16_tau2, com_17_tau1, com_17_tau2   ):
    params = {}    
...
#     params['exp_1_tau2'] = exp_1_tau2
    ...
    params['com_15_tau1'] = com_15_tau1
    params['com_15_tau2'] = com_15_tau2
    params['com_16_tau1'] = com_16_tau1
    params['com_16_tau2'] = com_16_tau2
    params['com_17_tau1'] = com_17_tau1
    params['com_17_tau2'] = com_17_tau2
    ...
    df_train = df_train.append({'com':15,'TH':com_15_tau1 , 'TH_2':com_15_tau2 }, ignore_index=True)
    df_train = df_train.append({'com':16,'TH':com_16_tau1 , 'TH_2':com_16_tau2 }, ignore_index=True)
    df_train = df_train.append({'com':17,'TH':com_17_tau1 , 'TH_2':com_17_tau2 }, ignore_index=True)
...
parser.add_argument(
    "--dataset",
    default="PHISON",
    choices=["CIFAR10", "CIFAR100", "PHISON","PHISON_fourcls",'PHISON_regroup','PHISON_good','PHISON_shift','PHISON_broke','PHISON_short'],
    help="Pick a dataset",
)
...
# train_df, val_df, _, _, _, _, _ = CreateDataset_relabel_sixcls(args["random_seed"], add_test)
# train_df, val_df, _, _, _, _, _ = CreateDataset_relabel_sixcls_randomsplit(args["random_seed"], add_test)
# train_val_df = pd.concat([train_df, val_df])
if args_due.dataset == 'PHISON_good':
    train_df, val_df, _, _, _, _, _,_ = CreateDataset_relabel_good(args["random_seed"], add_test)
if args_due.dataset == 'PHISON_short':
    train_df, val_df, _, _, _, _, _,_ = CreateDataset_relabel_short(args["random_seed"], add_test)
if args_due.dataset == 'PHISON_fourcls':
    train_df, val_df, _, _, _, _, _,_ = CreateDataset_relabel_fourcls(args["random_seed"], add_test)
# train_val_df = pd.concat([train_df, val_df])
