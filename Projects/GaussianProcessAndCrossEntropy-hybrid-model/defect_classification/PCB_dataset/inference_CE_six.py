...
from due import dkl_Phison_exp1
...
#         test_df = pd.concat([train_df, val_df])
        ...
        for name in set(test_df_mapping2_label['component_name'].values):
            ...
            for k, v in zip(temp_data['class'].value_counts().keys(), temp_data['class'].value_counts()):
                ...
                if k == 0:
                    ...
                    try:
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'missing'] = temp_data['class'].value_counts().values[num]
                    except:
                        print(f"{name} only contains bad label.")
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'missing'] = temp_data['class'].value_counts().values[0]
                elif k ==2:
                    try:
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'shift'] = temp_data['class'].value_counts().values[num]
                    except:
                        print(f"{name} only contains bad label.")
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'shift'] = temp_data['class'].value_counts().values[0]
                elif k ==3:
                    try:
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'stand'] = temp_data['class'].value_counts().values[num]
                    except:
                        print(f"{name} only contains bad label.")
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'stand'] = temp_data['class'].value_counts().values[0]
                elif k ==4:
                    try:
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'broke'] = temp_data['class'].value_counts().values[num]
                    except:
                        print(f"{name} only contains bad label.")
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'broke'] = temp_data['class'].value_counts().values[0]
                elif k ==5:
                    try:
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'short'] = temp_data['class'].value_counts().values[num]
                    except:
                        print(f"{name} only contains bad label.")
                        test_component_name_df.loc[test_component_name_df['component_name'] == name, 'short'] = temp_data['class'].value_counts().values[0]
                num=num+1
        ...
        #     test_component_name_df['shift'] = test_component_name_df['missing'].fillna(0).astype(int)
...
#                         df_all.loc[(df_all["component_name"] == name.item()), 'leakage'] +=1
    ...
    defect_types = ['good', 'missing', 'shift','stand','broke','short']
    cm = confusion_matrix(y_true_all, y_pred_all,labels=[0, 1, 2, 3, 4, 5])
    ...
    # 保存图片
    plt.savefig(f'./output/{args_due.output_inference_dir}/all_com_confusion_matrix.pdf', dpi=300, bbox_inches='tight')
        # plt.show()


    print(' * EXP1_Acc@1 {top1.avg:.3f}\n'.format(top1=top1))
    # ALL test set
    calculatePerformance(df_all, file_name=f'./output/baseline/{args_due.output_inference_dir}/DUE_dataset_{args_due.output_inference_dir}_EXP1_ALL_test_set_overkill_and_leakage.csv')
...
def EXP2(args_due ,model ,test_loader ,df ,train_component_label, val_component_label, test_component_label ,likelihood ,TH , TH_2,good_num_com):
    ...
    with torch.no_grad():
        for idx, (images, labels, _, component_name) in enumerate(test_loader):
            images = images.float().cuda()
            labels = labels.cuda()
...
#             import pdb;pdb.set_trace()
    ...
    calculatePerformance(df, file_name=f'./output/{args_due.output_inference_dir}/DUE_dataset_{args_due.output_inference_dir}_HBE_ALL_test_set_overkill_and_leakage_unk.csv')
...
if __name__ == "__main__":
    ...
    if not os.path.isdir('./output/baseline/'+args_due.output_inference_dir):
        os.makedirs('./output/baseline/'+args_due.output_inference_dir)
...
#     input_size, num_classes, train_dataset, test_dataset, train_loader, train_com_loader = ds
    ...
    print(num_classes)
    model  = set_model(args, args_due, train_com_loader , num_classes)
